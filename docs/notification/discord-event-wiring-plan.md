...
| 대상 | 권한 |
| --- | --- |
| Job·Program 조회/재시도 | 등록자 또는 담당 교사, 그리고 DEVELOPER |
| Inquiry 조회 | DEVELOPER |
| Inquiry 재시도 | **API를 만들지 않는다** — 명세에 없다 |

`SecurityConfig`의 경로 규칙(`admin/jobs/**`, `admin/programs/**`는 TEACHER·DEVELOPER)은 Role만 거른다. **소유권(등록자/담당 교사) 검증은 Service 계층이 별도로 수행**해야 한다 — `requireManager`와 동일한 판정이 필요하므로, Job/Program이 그 판정을 공개 Port로 노출하고 Notification이 소비한다(방향은 기존과 같은 `notification → 도메인`).
...
service/DiscordChannelResolverTest.kt         허용 목록 검증, 미등록 Key 거부, 학년 → Role 변환
service/impl/JobServiceImplTest.kt            (수정) DRAFT 수정·삭제 시 Event 미발행
service/impl/ProgramServiceImplTest.kt        (수정) 동일 + discordChannelId 허용 목록 거부
...
```text
원본 Transaction Rollback 시 Delivery가 생기지 않는다
원본 Commit + Bot 실패 시 원본 데이터가 유지된다
같은 Event가 두 번 도착해도 Delivery Row가 하나만 생긴다(idempotency_key UNIQUE)
```

Bot 호출은 `FakeDiscordBotClient`로 대체한다. 실제 Discord·실제 Bot을 호출하지 않는다.
