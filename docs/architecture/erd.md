...
- `member_job_preferences`: Surrogate PK(`id`, `GENERATED BY DEFAULT AS IDENTITY`) + `uk_member_job_preferences_member_job UNIQUE (member_id, job_id)`(V24 Migration, Recommendation Notion 계약 정합성 Issue #155 -- `GET/POST/DELETE /api/v1/me/recommendation-exclusions`가 안정적인 Long `exclusionId`를 요구해 기존 복합 PK에서 전환)
...
member_roles 복합키, member_job_preferences Surrogate PK + (member_id, job_id) UNIQUE(V24), job_ai_analyses 공유 PK
