...
- 불필요한 Dependency
- 남아 있는 Debug Code
- 새 Package를 추가했다면 다른 Application Module의 내부 구현을 직접 참조하지 않는지, 순환 의존을 만들지 않는지 ([`docs/architecture/modularity.md`](../../../docs/architecture/modularity.md) 참고)
...
이 저장소는 아직 도메인 로직, 실제 Domain API, Security가 구현되어 있지 않다. PostgreSQL/Redis 연결과 Migration/Test 기반([`docs/development/persistence.md`](../../../docs/development/persistence.md)), 공통 Web/API 응답·오류 처리 기반([`docs/development/web-api.md`](../../../docs/development/web-api.md))은 구성되어 있지만 실제 Domain Entity/Repository/Controller는 아직 없다. 관련 항목은 실제로 구현이 생겼을 때부터 적용하고, 존재하지 않는 구조를 문제로 지적하지 않는다.
